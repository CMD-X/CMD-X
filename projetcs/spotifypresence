if not syn then Message = Instance.new("Message"); Message.Text = "This script can only be run on Synapse X, x.synapse.to"; wait(5); Message:Destroy(); return end
...
SpotifyMain.Position = UDim2.new(0, 750, 0, 450)
...
SpotifyMain.Active = true
SpotifyMain.Draggable = true
...
local SongTitle = Instance.new("TextLabel", TokenLock)
...
SongTitle.Text = "Spotify Presence"
...
local SongArtist = Instance.new("TextLabel", TokenLock)
...
function MainFrame()
    ...
    local API = function(URL, method)
        ...
        if success == true and type(res) == "table" and #res.Body > 0 then
            ...
            if parsed['error'] then ExpiredToken() end
            if parsed['item'] == nil then
                return {
...
function ExpiredToken()
    ...
    SongTitle.Parent = TokenLock
    SongArtist.Parent = TokenLock
    wait(1.5)
end
...
SpotifyLogo.MouseButton1Down:Connect(function()
    if TokenLock.Visible then
        ...
        wait(1.5)
        TokenBox.Text = " "
        SongTitle.Parent = SpotifyMain
        SongArtist.Parent = SpotifyMain
        wait(1)
